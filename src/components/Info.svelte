<div class="hero min-h-[calc(100vh-80px)] lg:min-h-[calc(100vh-136px)] bg-base-100 px-4">
	<div class="hero-content text-start">
		<div class="max-w-md">
			<h1 class="text-5xl font-bold text-primary">Frontend Developer.</h1>
			<p class="py-8">
				I like to create modern and intuitive interfaces with small details that make up for a <span
					class="font-bold">great</span
				> user experience.
			</p>
			<div class="flex w-full pt-4 pb-8">
				<div class="w-1/2">I like robust design systems and progressive enhancement.</div>
				<div class="divider divider-horizontal" />
				<div class="w-1/2">Currently doing Bsc. in Applied Data Technology at OsloMet.</div>
			</div>
			<button class="btn btn-secondary  rounded-lg btn-block">Get in touch</button>
		</div>
	</div>
</div>
